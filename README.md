...
**Cyber Hub Information Tech.** A portable IT toolbox in one file. Download it, double-click it, get
your answer.

CHIT is the set of small network and diagnostic tools an IT tech reaches for every day, wrapped in
...
Press `Ctrl+K` (`Cmd+K` on macOS) anywhere in the app to search every tool. Type the tool's name,
or the command it replaces: `nslookup`, `ipconfig`, `tracert`, `netcat`.

Three things to know about this build. **Traceroute needs the system tool installed**: Windows and
macOS ship it, but a minimal Linux install may not, and CHIT will tell you the exact command to
install it. **Three tools accept an incoming connection** while they are running, and only while
they are running: LAN File Drop, Port Listener and LAN Throughput Test. Each shows a Stop button the
whole time it is listening; see the firewall note below. **Only the Linux build has been run**; the
Windows and macOS binaries compile from the same source but nobody has launched one yet, and the
Windows-only parts of System Info, Startup and Services, USB Device History, Listening Ports,
Installed Software List, Wi-Fi Connection Info, Battery Health and Disk Speed Test have never
executed.
...
This is expected: **v1.0 ships unsigned, deliberately.** An Authenticode certificate costs a few
hundred dollars a year and an EV one more, and even a signed binary carries the same warning until
it has built up SmartScreen reputation. For a tool handed between colleagues who already trust where
they got it from, the warning is the honest state of affairs rather than a problem to paper over. If
CHIT is ever handed to people outside the team, buy the certificate then. The same reasoning
applies to the macOS notarisation note below.
...
## First run, and checking for updates
...
One entry in `frontend/src/tools/registry.ts` plus a folder at `frontend/src/tools/<id>/` with a
default-exported React component. The sidebar, home screen, command palette and routing all read
from that registry. Backend methods hang off `*App` in the repo root, in their own `app_<tool>.go`
file, and use `a.jobs` for anything long running.

The shared pieces are worth reading before writing anything: `internal/core` (the job engine, the
worker pool and the error codes), `internal/store` (namespaced JSON persistence),
`frontend/src/components/` (`ToolShell`, `ResultsTable`, `Button`, `TextInput`, `Textarea` and
friends) and `frontend/src/lib/useJob.ts`. `internal/tools/ipscan/` with
`frontend/src/tools/ip-range-scanner/` is the reference implementation for a streaming job, and
`app_mac.go` with `frontend/src/tools/mac-lookup/` for a simple request and response.

The full internal conventions, the roadmap and the per-tool specifications are kept out of this
